...
/**
 * REST controller for managing {@link com.apbiomedicale.domain.Boitier}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BoitierResource {
    ...
    public ResponseEntity<Boitier> updateBoitier(@PathVariable(value = "id", required = false) final Long id, @RequestBody Boitier boitier)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, boitier.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Boitier> partialUpdateBoitier(
        ...
        Optional<Boitier> result = boitierRepository
            ...
            .map(existingBoitier -> {
                ...
                if (boitier.getRef() != null) {
                    existingBoitier.setRef(boitier.getRef());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, boitier.getId().toString())
        );
    }
